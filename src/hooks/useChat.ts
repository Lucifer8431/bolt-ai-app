...
export function useChat() {
    ...
    async (
      ...
      if (senderId === "user") {
        ...
        try {
          ...
          const randomMember =
            aiMembers[Math.floor(Math.random() * aiMembers.length)];
          ...
          // Save AI response to database (only if not in demo mode)
          ...
          // Check if it's an API key issue or other error
          if (!aiService.isConfigured()) {
            toast.error(
              "Please configure your OpenAI API key in Settings to enable AI responses.",
            );
          } else {
            toast.error(
          ...
          const fallbackMessage: Message = {
            ...
            senderId: "1",
            content:
              "I apologize, but I cannot respond right now. Please configure your OpenAI API key in Settings for full AI capabilities.",
            timestamp: new Date(),
            type: "text",
          };
  ...
  const generateCode = useCallback(
    async (prompt: string, language: string = "javascript") => {
      try {
        if (user?.id) {
          await aiService.loadUserApiKeys(user.id);
        }
